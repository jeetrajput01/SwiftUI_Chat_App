...
extension HomeViewModel {
    ...
    func changeActivityStatus(makeUserInActive:Bool = true , completion: @escaping () -> Void) {
        ...
        if currentUserID == "SDF" {
            print("get userID form changeActivity \(currentUserID)")
            return
        }
...
extension HomeViewModel {
    ...
    func getUserIDFormCollection(completion: @escaping (Bool , [String],[ConversationModel]?) -> Void) {
        ...
        let currentUserID = UserModel.getCurrentUserID()
        if currentUserID == "SDF" {
            print("get userID form getUserID form collection \(currentUserID)")
            return
        }
                ...
                self.db.collection(Constant.FireBase.userCollection).whereField(kUID, in: userID).addSnapshotListener { (QuerySnapshot, error) in
                    if error != nil {
                        ...
                        for i in 0...arrCov.count - 1 {
                            ...
                            if UserModel.getCurrentUserID() == "SDF" {
                                print("get userID form getChatUsers \(UserModel.getCurrentUserID())")
                                return
                            }
    ...
    func getOtherUsers(completion: @escaping () -> Void) {
        ...
        if !self.userList.isEmpty {
            ...
            arrUsersUID = arrUsersUID.filter({$0 != "SDF"})
    ...
    func getAllUsers() {
        ...
        db.collection(Constant.FireBase.userCollection).addSnapshotListener { (querySnapshot, error) in
            ...
            /// remove current user
            var tempArr = self.otherUsers
            tempArr = tempArr.filter({$0.uID != "SDF"})
    ...
    func removeUnwantedData() {
        self.db.collection(Constant.FireBase.userCollection).document("SDF").delete()
    }
