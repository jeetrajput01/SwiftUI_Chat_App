...
class LoginViewModel: ObservableObject {
    ...
    @Published var openGoogleSheet = false
            ...
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason) { (success, error) in
                if success {
                    DispatchQueue.main.async {
                        ...
                        self.moveToHomeScreenWIthEmailLogin()
...
//MARK: - firebase auth with facebook
extension LoginViewModel {
...
extension LoginViewModel {
    ...
    /// - Parameters:
    ///   - creadital: google auth credential
    ///   - success:escaping with success message and auth data
    ///   - faliure: escapping with failure error
    func loginWIthGoogleFacebook(credential: AuthCredential, success: @escaping (_ message:String, _ authData: AuthDataResult?) -> (), faliure: @escaping (_ error: String) -> Void) {
        
        Auth.auth().signIn(with: credential) { (user, error) in
            
            if error != nil {
                faliure(error?.localizedDescription ?? "google auth error")
                return
            } else {
    ...
    func moveToHomeWIthGoogle_FacebookLogin(credential: AuthCredential) {
        
        showLoadingIndicator = true
        loginWIthGoogleFacebook(credential: credential) { (sucess, user) in
            ...
            self.db.collection(Constant.FireBase.userCollection).document(uID).getDocument { (DocumentSnapshot, error) in
                ...
                if error != nil {
                    ...
                    if let document = DocumentSnapshot {
                        ...
                        if document.exists {
                           ...
                           // create new document
                           guard let displayName = user?.user.displayName else {
                               self.errorMessage = "unable to get data form server \n please try again later"
                               self.showingError = true
                               return
                           }
                        ...
                        self.showLoadingIndicator = false
                        self.errorMessage = "unable to get data user data form server"
                        self.showingError = true
                    }
...
extension LoginViewModel {
    ...
    func moveToHomeScreenWIthEmailLogin() {
